...
fs.writeFileSync(productsFilePath,  JSON.stringify(products, null, 3), 'utf8')
	...
	update: (req, res) => {
		...
		fs.writeFileSync(productsFilePath, JSON.stringify(productEdit, null, 3), 'utf8')
	...
	destroy : (req, res) => {
		const productRemove = products.filter(product => product.id !== +req.params.id)
		
		fs.writeFileSync(productsFilePath, JSON.stringify(productRemove, null, 3), 'utf8')
